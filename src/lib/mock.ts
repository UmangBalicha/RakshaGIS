/* Demo-mode backend: localStorage-backed store with seeded data.
 * Used automatically when Supabase env vars are missing, so the app
 * runs with zero setup. Mirrors the Supabase schema 1:1.
 */
import type {
  AlertLog,
  AppNotification,
...
function load(): MockDB {
  try {
    ...
    if (raw) {
      ...
      if (parsed.version === 3 && Array.isArray(parsed.reports)) return parsed;
    }
...
const asyncWrap = async <T>(v: T): Promise<T> => v;

/* ---------------- Session ---------------- */

export function mockGetSessionId(): string | null {
  try {
...
export function mockSetSessionId(id: string | null): void {
  try {
...
export function mockGetSessionProfile(): Promise<Profile | null> {
  const id = mockGetSessionId();
  if (!id) return asyncWrap(null);
...
export async function mockSignIn(email: string, password: string): Promise<Profile> {
  ...
  if (!found) throw new Error('No account found for this email. Please register first.');
...
export async function mockSendOtp(phone: string): Promise<void> {
...
export async function mockVerifyOtp(input: {
  ...
  if (input.code.trim() !== expected) throw new Error('Incorrect code. Hint: demo code is 123456.');
  const store = db();
...
export function mockListReports(): Promise<DisasterReport[]> {
  const rows = [...db().reports].sort((a, b) => b.created_at.localeCompare(a.created_at));
  return asyncWrap(rows);
}

export function mockGetReport(id: string): Promise<DisasterReport | null> {
  return asyncWrap(db().reports.find((r) => r.id === id) ?? null);
}

export function mockCreateReport(
...
export function mockUpdateReport(
  id: string,
  ...
  if (patch.severity) report.severity = patch.severity;
...
export function mockListZones(): Promise<SafeZone[]> {
  const rows = [...db().zones].sort((a, b) => a.name.localeCompare(b.name));
  return asyncWrap(rows);
}

export function mockCreateZone(input: NewSafeZoneInput): Promise<SafeZone> {
  const store = db();
  const now = new Date().toISOString();
  const zone: SafeZone = {
    id: uid('zone'),
...
export function mockUpdateZone(
  id: string,
  patch: Partial<Pick<SafeZone, 'name' | 'type' | 'address' | 'capacity' | 'current_occupancy' | 'amenities' | 'is_active' | 'latitude' | 'longitude'>>,
): Promise<SafeZone> {
  const store = db();
...
export function mockDeleteZone(id: string): Promise<void> {
  const store = db();
...
export function mockListAlerts(limit = 50): Promise<AlertLog[]> {
  return asyncWrap([...db().alerts].slice(0, limit));
}

export function mockListNotifications(userId: string): Promise<AppNotification[]> {
  const rows = db()
...
export function mockMarkNotificationRead(id: string): Promise<void> {
...
export function mockListProfiles(): Promise<Profile[]> {
  return asyncWrap([...db().profiles]);
}

export function mockSetRole(id: string, role: Role): Promise<Profile> {
  const store = db();
...
export function mockResetDemo(): void {
  mem = seed();
  save();
}
