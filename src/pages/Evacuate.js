...
export default function Evacuate() {
    ...
    if (loading && reports.length === 0) {
        return (_jsxs("div", { className: "space-y-3", role: "status", children: [_jsx("div", { className: "h-12 animate-pulse rounded-xl bg-slate-200" }), _jsx("div", { className: "h-64 animate-pulse rounded-xl bg-slate-200" }), _jsx("div", { className: "h-32 animate-pulse rounded-xl bg-slate-200" })] }));
    }
    if (loadError && !report) {
        return (_jsxs("div", { className: "space-y-3", children: [_jsxs(Link, { to: "/", className: "inline-flex min-h-[44px] touch-manipulation items-center gap-1.5 py-2 text-sm font-bold text-slate-600 hover:text-slate-900", children: [_jsx(ArrowLeft, { className: "h-4 w-4" }), " Back to map"] }), _jsxs("div", { role: "alert", className: "rounded-xl border border-red-200 bg-red-50 p-4 text-center", children: [_jsx("p", { className: "text-sm font-bold text-red-800", children: "Couldn\u2019t load this incident \u2014 you may be offline." }), _jsx(Button, { size: "sm", className: "mt-3", onClick: () => void refresh(), children: "Retry" })] })] }));
    }
    if (!report) {
    ...
    return (_jsxs("div", { className: "space-y-3", children: [_jsxs("div", { className: "sticky top-[calc(3.5rem+env(safe-area-inset-top))] z-[900] -mx-4 flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 bg-slate-100/95 px-4 py-2 backdrop-blur sm:top-[calc(4rem+env(safe-area-inset-top))]", children: [_jsxs(Link, { to: "/", className: "inline-flex min-h-[44px] touch-manipulation items-center gap-1.5 py-2 text-sm font-bold text-slate-600 hover:text-slate-900", children: [_jsx(ArrowLeft, { className: "h-4 w-4" }), " Back to map"] }), _jsx(Link, { to: "/report", className: "inline-flex min-h-[48px] touch-manipulation items-center justify-center gap-2 rounded-lg bg-brand-600 px-5 text-sm font-semibold text-white shadow-sm hover:bg-brand-700 active:scale-[0.98]", children: "Report an incident \u2192" })] }), _jsxs("div", { className: "flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3", children: [report.severity === 'critical' || report.severity === 'high' ? (_jsx(AlertTriangle, { className: "h-5 w-5 shrink-0 text-red-600" })) : (_jsx(MapPin, { className: "h-5 w-5 shrink-0 text-slate-500" })), _jsxs("div", { className: "min-w-0 flex-1", children: [_jsxs("p", { className: "text-sm font-extrabold text-slate-900", children: ["Evacuate \u2014 ", typeLabel] }), _jsx("p", { className: "truncate text-sm text-slate-500", children: report.address })] }), _jsx("span", { className: cn('inline-flex items-center rounded-full px-2.5 py-1 text-xs font-bold ring-1 ring-inset', SEVERITY_META[report.severity].badge), children: SEVERITY_META[report.severity].label })] }), _jsx(IncidentMap, { reports: [report], zones: zones, redZones: redZones, showDangerZones: true, selectedRoute: selectedRoute, className: "rg-map-evac", userDot: true }), _jsx(EvacuationPanel, { origin: { lat: report.latitude, lng: report.longitude, label: report.address }, onRouteSelect: setSelectedRoute })] }));
}
