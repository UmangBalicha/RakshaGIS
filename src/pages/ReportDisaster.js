...
import { Button, Card, CardContent, Input, Label, Modal, Textarea } from '../components/ui';
...
export default function ReportDisaster() {
    ...
    // Revoke the *previous* batch whenever photos change, plus on unmount —
    // the old []-dep cleanup closed over the first empty array and leaked
    // every blob URL for the session.
    useEffect(() => () => {
        previews.forEach((u) => URL.revokeObjectURL(u));
    }, [previews]);
    // Resolve the address in the background whenever the pin moves —
    // never blocks the form (network can be slow during disasters). A
    // hand-edited address is never clobbered by a late resolve.
    ...
    useEffect(() => {
        ...
        void reverseGeocode(location.lat, location.lng).then((addr) => {
            if (!cancelled && !addressTouched.current) {
                setAddress(addr);
    ...
    const canSubmit = location !== null &&
        ...
        (disasterType === 'other' ? description.trim().length > 0 : true);
    // A half-filled report is precious: block in-app navigation away (bottom
    // tabs, logo, back links) with a confirm dialog, and warn on tab close /
    // reload. Refs (not state) so the success navigation after submit can
    // flip the guard off without waiting for a re-render.
    const dirtyRef = useRef(false);
    dirtyRef.current = location !== null || disasterType !== null || description.trim() !== '' || hasInjuries !== null || photos.length > 0;
    const sentRef = useRef(false);
    const blocker = useBlocker(() => dirtyRef.current && !sentRef.current);
