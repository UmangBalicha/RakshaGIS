...
export default function AdminDashboard() {
    ...
    return (_jsxs("div", { className: "space-y-4", children: [_jsxs("div", { className: "flex flex-wrap items-center justify-between gap-3", children: [_jsxs("div", { children: [_jsx("h1", { className: "text-2xl font-extrabold text-slate-900", children: "Authority console" }), _jsx("p", { className: "mt-0.5 text-sm text-slate-500", children: "Live all-hazard intelligence, triage and evacuation tracking." })] }), _jsxs("div", { className: "flex gap-2", children: [_jsx(Link, { to: "/admin/reports", className: "inline-flex min-h-[48px] touch-manipulation items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50 active:scale-[0.98]", children: "Manage reports" }), _jsx(Link, { to: "/admin/safe-zones", className: "inline-flex min-h-[48px] touch-manipulation items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50 active:scale-[0.98]", children: "Safe zones" })] })] }), _jsxs("div", { className: "grid grid-cols-2 gap-3 lg:grid-cols-5", children: [_jsx(Stat, { label: "Total reports", value: stats.total, accent: "bg-slate-500" }), _jsx(Stat, { label: "Active incidents", value: stats.active, accent: "bg-orange-500" }), _jsx(Stat, { label: "Critical active", value: stats.critical, accent: "bg-red-600" }), _jsx(Stat, { label: "Reported today", value: stats.today, accent: "bg-blue-500" }), _jsx(Stat, { label: "Safe zones live", value: zones.filter((z) => z.is_active).length, accent: "bg-emerald-500" })] }), _jsx(ReportFiltersBar, {}), _jsxs("div", { className: "grid grid-cols-1 gap-4 xl:grid-cols-3", children: [_jsxs(Card, { className: "xl:col-span-2", children: [_jsxs(CardHeader, { className: "flex flex-row items-center justify-between", children: [_jsxs(CardTitle, { children: ["Live incident map (", active.length, " active)"] }), _jsxs("span", { className: "inline-flex items-center gap-1.5 text-xs font-bold text-emerald-600", children: [_jsx("span", { className: "h-2 w-2 animate-pulse rounded-full bg-emerald-500" }), " LIVE"] })] }), _jsx(CardContent, { children: loading && reports.length === 0 ? (_jsx("div", { className: "flex justify-center py-16", children: _jsx(Spinner, {}) })) : loadError && reports.length === 0 ? (_jsxs("div", { role: "alert", className: "rounded-xl border border-red-200 bg-red-50 p-4 text-center", children: [_jsx("p", { className: "text-sm font-bold text-red-800", children: "Couldn\u2019t load live data \u2014 check your connection." }), _jsx(Button, { size: "sm", className: "mt-3", onClick: () => void refresh(), children: "Retry" })] })) : active.length === 0 ? (_jsx(EmptyState, { title: "No active incidents", hint: "New reports will appear here in real time." })) : (_jsx(IncidentMap, { reports: active, zones: zones, showDangerZones: true, height: "480px", fitKey: `admin-${filtered.length}`, focus: focus, onViewDetails: (reportId) => {
                                        const r = reports.find((x) => x.id === reportId);
                                        if (r)
                                            setSelected(r);
